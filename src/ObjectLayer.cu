...
__global__ void CTdetforward_kernel(const float *hm, const float *reg,const float *wh ,
    ...
    for (cls = 0; cls < classes; ++cls )
    {
        ...
        if((max_index == objIndex) && (objProb > visthresh)){
            ...
            //printf("%d",resCount);
            char* data = (char * )output + sizeof(float) + resCount*sizeof(Object_Detection::Detection);
            Object_Detection::Detection* det =  (Object_Detection::Detection*)(data);
...
void CTdetforward_gpu(const float *hm, const float *reg,const float *wh ,float *output,
    ...
    uint num = w * h;
    CTdetforward_kernel<<<Object_Detection::cudaGridSize(num),BLOCK>>>(hm,reg,wh,output,w,h,classes,kernerl_size,visthresh);
}
